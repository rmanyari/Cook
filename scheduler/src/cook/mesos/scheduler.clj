...
(defn make-offer-handler
  [conn driver-atom fenzo framework-id executor-config pending-jobs-atom offer-cache
   ...
   trigger-chan mesos-run-as-user]
  (let [chan-length 100
        ...
        (let [num-considerable @fenzo-num-considerable-atom
              ...
              (try
                (let [
                      ...
                      matched-head? (handle-resource-offers! conn @driver-atom fenzo framework-id executor-config pending-jobs-atom mesos-run-as-user @user->usage-future user->quota num-considerable offers-chan offers)]
                  (when (seq offers)
...
(defn create-datomic-scheduler
  [{:keys [conn driver-atom executor-config fenzo-fitness-calculator fenzo-floor-iterations-before-reset
           ...
           pending-jobs-atom progress-config sandbox-syncer-state task-constraints trigger-chans]}]
